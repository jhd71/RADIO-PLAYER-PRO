// api/nowplaying.js - API Vercel pour récupérer les métadonnées radio (VERSION 3)
        ...
        if (icyMetaInt) {
            ...
            if (buffer.length > metaInt) {
                ...
                if (metaLength > 0 && buffer.length >= metaInt + 1 + metaLength) {
                    ...
                    // Chercher StreamTitle avec un meilleur regex
...
function cleanTitle(title, stationName) {
    ...
    // Vérifier si c'est juste le nom de la station (comparaison partielle)
    if (stationName && cleaned.toLowerCase().includes(stationName.toLowerCase())) {
        // Si c'est JUSTE le nom de la station, ignorer
